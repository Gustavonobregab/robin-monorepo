'use client'
import Link from 'next/link'
import { FileText, Music, Image as ImageIcon } from 'lucide-react'

const actions = [
  { href: '/dashboard/text', icon: FileText, label: 'Compress Text', description: 'Clean up and convert JSON to TOON' },
  { href: '/dashboard/audio', icon: Music, label: 'Compress Audio', description: 'Remove silence, normalize' },
  { href: '/dashboard/image', icon: ImageIcon, label: 'Compress Image', description: 'WebP and AVIF compression' },
]

export function QuickActions() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {actions.map(({ href, icon: Icon, label, description }) => (
        <Link
          key={href}
          href={href}
          className="bg-background rounded-xl p-4 sm:p-5 border border-border shadow-sm flex items-center gap-4 transition-colors hover:border-accent-strong"
        >
          <div className="w-9 h-9 rounded-xl bg-accent-light flex items-center justify-center flex-shrink-0">
            <Icon className="w-4 h-4" />
          </div>
          <div className="min-w-0">
            <p className="font-medium text-sm">{label}</p>
            <p className="text-xs text-muted truncate">{description}</p>
          </div>
        </Link>
      ))}
    </div>
  )
}
