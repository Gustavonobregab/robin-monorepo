...
Instead of polling, Robin Wood can `POST` to your server when a job finishes. Webhooks require a plan that includes them (Pro); on Free, passing `webhookUrl` returns `FEATURE_NOT_AVAILABLE` (403).
