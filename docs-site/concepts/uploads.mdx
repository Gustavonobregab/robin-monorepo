---
title: "Uploads"
description: "Presigned uploads for audio and document files"
---
...
4. Use `id` as `audioId` (audio jobs) or `fileId` (text jobs).
...
| Constraint | Value |
| --- | --- |
| Allowed formats | `.mp3`, `.wav`, `.pdf`, `.txt` |
...
Content types are assigned by extension: `audio/mpeg`, `audio/wav`, `application/pdf`, `text/plain`.
...
- the file's magic bytes match a valid MP3/WAV/PDF signature (`INVALID_FORMAT`, 422). Plain-text files skip this check.
