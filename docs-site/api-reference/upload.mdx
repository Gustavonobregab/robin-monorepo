...
<ParamField body="filename" type="string" required>
  Original filename, 1–255 characters. The extension determines the content type and must be one of `.mp3`, `.wav`, `.pdf`, `.txt`.
</ParamField>
...
<ResponseField name="id" type="string">
  Upload id — pass as `audioId` (audio jobs) or `fileId` (text jobs).
</ResponseField>
