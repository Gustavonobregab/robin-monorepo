...
Create an asynchronous audio compression [job](/concepts/jobs) from an uploaded `.mp3` or `.wav` file.
...
- Output is **`.ogg` (Opus)** by default — best quality per bitrate for voice. Pass `{"type": "encode", "params": {"format": "mp3"}}` for `.mp3` when you need broad compatibility.
...
Poll `GET /v1/jobs/:id` until `status` is `completed`, then download `result.outputUrl` (signed, 1-hour validity, regenerated on every read). Results are retained for 7 days.
