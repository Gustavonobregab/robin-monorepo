...
<ParamField query="type" type="string">
  Filter by pipeline: `audio`, `text`, `image`, `video`. (`image` and `video` are reserved for upcoming pipelines.)
</ParamField>
...
<ResponseField name="items" type="array">
  Job summaries: `id`, `type`, `status`, `name` (original filename, audio jobs), `error` (failed jobs), `metrics` (completed jobs), `createdAt`, `completedAt`.
</ResponseField>
...
```bash
curl -s "https://api.robinwood.dev/v1/jobs?type=audio&status=completed&limit=2" \
