...
Create a key in the dashboard under **Settings → API Keys**. The full key (`sk_live_...`) is shown once — copy it immediately; only a hash is stored.
...
<Step title="Poll the job and download the result">
...
While running, `status` moves through `created → pending → processing`. When it's `completed`:
