...
Robin Wood compresses content for LLM pipelines and storage. Send text or audio, pick a preset (or compose your own operation chain), get back a smaller output.

## What it does

- **Text** — whitespace/punctuation/unicode cleanup, and conversion of large JSON blocks to TOON tabular notation. Small inline inputs are processed synchronously; larger ones run as jobs.
...
4. **Get the result** by polling `GET /v1/jobs/:id` or receiving a signed webhook.
